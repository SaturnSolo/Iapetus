...
public class TextResponses extends ListenerAdapter {
	...
	public void onMessageReceived(MessageReceivedEvent event) {
        ...
        switch (content) {
            ...
            case String s when s.contains("🍓") -> output.append(STRAWBERRY.get(rng.nextInt(STRAWBERRY.size()))).append("\n");
            case String s when s.contains("🍒") -> output.append(CHERRY.get(rng.nextInt(CHERRY.size()))).append("\n");
            case String s when s.contains("\uD83E\uDED0") -> output.append(BLUEBERRY.get(rng.nextInt(BLUEBERRY.size()))).append("\n");
            case String s when s.contains("moss") -> output.append(MOSS.get(rng.nextInt(MOSS.size()))).append("\n");
            case String s when (s.equals("iapetus") || s.contains("<@%s>".formatted(event.getJDA().getSelfUser().getId()))) -> output.append(IAPETUS.get(rng.nextInt(IAPETUS.size()))).append("\n");
            default -> {}
        }
