...
public class TextResponses extends ListenerAdapter {
	private final static List<String> blueberry = Arrays.asList(
	...
	private static final List<String> strawberry = Arrays.asList("Look a strawberry!", "Strawberries are my favorite!",
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (content.contains("🍓"))
			channel.sendMessage(getStrawberries()).queue();
		if (content.contains("🍒"))
			channel.sendMessage(getCherries()).queue();
		if (content.contains("\uD83E\uDED0"))
			channel.sendMessage(getBlueberries()).queue();
		if (content.toLowerCase().contains("moss"))
			channel.sendMessage(getMoss()).queue();
