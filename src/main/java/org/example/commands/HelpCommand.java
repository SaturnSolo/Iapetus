...
public class HelpCommand extends IapetusCommand {
	...
	public HelpCommand(ButtonManager buttonMgr) {
		...
		buttonMgr.addButtons(new IapetusButton(Button.primary("commands", "💚")) {
			@Override
			public void run(ButtonInteractionEvent event) {
				MessageEmbed embed = new EmbedBuilder().setTitle("**`Commands`**")
						.setDescription(
								"""
			...
			@Override
			public void run(ButtonInteractionEvent event) {
				MessageEmbed embed = new EmbedBuilder().setTitle("**`More Info`**")
						.setDescription(
								"""
	...
	public boolean runCommand(SlashCommandInteractionEvent event) {
		MessageEmbed embed = new EmbedBuilder().setTitle("**Iapetus**").setDescription(
				...
				.setThumbnail(
						"https://media.discordapp.net/attachments/1274910028888932402/1275871665603088465/iapetusnobackground.png?ex=66c77795&is=66c62615&hm=dd21eb064d66624006faed7bbab8435dbb030cdd2fa3847bcbe2557df9100ba8&=&format=webp&quality=lossless")
				.build();
		event.replyEmbeds(embed).addComponents(ActionRow.of(buttonMgr.getButton("commands"),
